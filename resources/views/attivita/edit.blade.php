...
@section('page-title', 'Modifica Attività')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-6">Modifica attività</h2>
        ...
        <form method="POST" action="{{ route('attivita.update', $attivita->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="tipo" required
                        ...
                        <option value="chiamata" {{ old('tipo', $attivita->tipo) === 'chiamata' ? 'selected' : '' }}>Chiamata</option>
                        <option value="email" {{ old('tipo', $attivita->tipo) === 'email' ? 'selected' : '' }}>Email</option>
                        <option value="incontro" {{ old('tipo', $attivita->tipo) === 'incontro' ? 'selected' : '' }}>Incontro</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="stato" required
                            class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-blue-500">
                        <option value="da_fare" {{ old('stato', $attivita->stato) === 'da_fare' ? 'selected' : '' }}>Da fare</option>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="data"
                           value="{{ old('data', $attivita->data->format('Y-m-d\TH:i')) }}" required
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="cliente_id"
                        ...
                        <option value="{{ $c->id }}" {{ old('cliente_id', $attivita->cliente_id) == $c->id ? 'selected' : '' }}>{{ $c->nome }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="opportunita_id"
                        ...
                        <option value="{{ $opp->id }}" {{ old('opportunita_id', $attivita->opportunita_id) == $opp->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="note" rows="3"
                          class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-blue-500">{{ old('note', $attivita->note) }}</textarea>
            </div>
